...
const BaseHead: FC<Props> = ({
  ...
  const description =
    title && _description ? `${title} ${_description}` : _description
  ...
  return (
    <>
      ...
      breadcrumbJsonLdItemListElements.length > 0 ? (
        <BreadcrumbJsonLd itemListElements={breadcrumbJsonLdItemListElements} />
      ) : (
        ''
      )}
      ...
      carouselJsonLdData.length > 0 ? (
        <CarouselJsonLd type="default" data={carouselJsonLdData} />
      ) : (
        ''
      )}
    </>
